# Build the UDFPS fingerprint-position runtime resource overlay (RRO).
...
& "$BT\apksigner.bat" sign `
    ...
    --out "$ROOT\udfps-overlay.apk" "$OUT\aligned.apk"
if ($LASTEXITCODE -ne 0) { throw "apksigner failed" }

$size = (Get-Item "$ROOT\udfps-overlay.apk").Length
Write-Host ""
Write-Host "BUILT: udfps-overlay.apk ($size bytes)"
