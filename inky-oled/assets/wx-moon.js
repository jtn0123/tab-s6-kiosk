  ...
  var moon = {
    ...
    paintPanel: function () {
      ...
      WP.qs("[data-body]", panel).innerHTML =
        ...
        + '<div class="big-sub">' + esc(m.name) + "</div></div></div>"
        + section("Cycle", statGrid([
            ["Illuminated", Math.round(m.frac * 100) + "%"],
            ["Moon age", (Math.round(m.age * 10) / 10) + " days", "of " + Math.round(SYNODIC * 10) / 10],
            ["Next full moon", shortDate(m.nextFull)],
            ["Next new moon", shortDate(m.nextNew)]
          ]));
    }
  };
